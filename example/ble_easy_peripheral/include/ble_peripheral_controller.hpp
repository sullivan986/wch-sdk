...
struct service_causality
{
    ...
    // notify/indicate index
    size_t notify_or_indicate_index;
...
class ble_peripheral_controller
{
  public:
    ...
    {
        service_operator(ble_peripheral_controller *parent, uint8_t set_id) : _parent(parent), _service_id(set_id)
        {
            service_map[set_id] = {};
            target_service = &service_map[set_id];
        }
    ...
    service_operator add_service_by_id(uint8_t service_id)
    {
        service_operator _service_operator(this, service_id);
        ...
        for (auto &&[service_id, service] : service_map)
        {
            for (auto &&[characteristic_id, characteristic] : service.characteristics)
            {
                ...
                if (characteristic.flag_indicate || characteristic.flag_notify)
                {
                    ...
                    service.notify_or_indicate_index = service.gatt_profile_table.size();
  ...
  private:
    ...
    template <class T> void service_init()
    {
        for (auto &&[service_id, service] : service_map)
        {
            for (auto &&[characteristic_id, characteristic] : service.characteristics)
    ...
    // lib CB 读 Callback
    static bStatus_t simpleProfile_ReadAttrCB(uint16_t connHandle, gattAttribute_t *pAttr, uint8_t *pValue,
    ...
    // lib CB 写 Callback
    static bStatus_t simpleProfile_WriteAttrCB(uint16_t connHandle, gattAttribute_t *pAttr, uint8_t *pValue,
                                               uint16_t len, uint16_t offset, uint8_t method)
    {
            ...
            for (auto &&[service_id, service] : service_map)
            {
                for (auto &&[characteristic_id, characteristic] : service.characteristics)
                {
                    if (uuid == characteristic.uuid)
                    {
                        ...
                        if (status == SUCCESS)
                        {
                            tmos_memcpy(pAttr->pValue, pValue, len > characteristic.buff.size());
                            notifyApp = characteristic_id;
                        }
                        find_flag = true;
                    }
                }
        ...
        if ((notifyApp != 0xFF))
        {
            simpleProfileChangeCB(notifyApp, pValue, len);
        }
