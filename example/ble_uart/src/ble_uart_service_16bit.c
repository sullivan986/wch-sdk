...
const uint8_t ble_uart_ServiceUUID[ATT_BT_UUID_SIZE] = {0xf0, 0xff};

// Characteristic rx uuid
const uint8_t ble_uart_RxCharUUID[ATT_BT_UUID_SIZE] = {0xf2, 0xff};

// Characteristic tx uuid
const uint8_t ble_uart_TxCharUUID[ATT_BT_UUID_SIZE] = {0xf1, 0xff};
...
static gattAttribute_t ble_uart_ProfileAttrTbl[] = {
    ...
    {{ATT_BT_UUID_SIZE, ble_uart_TxCharUUID}, 0, 0, (uint8_t *)&ble_uart_TxCharValue},
    ...
    {{ATT_BT_UUID_SIZE, ble_uart_RxCharUUID}, GATT_PERMIT_WRITE, 0, &ble_uart_RxCharValue[0]},
 ...
 * LOCAL FUNCTIONS
 */
static bStatus_t ble_uart_ReadAttrCB(uint16_t connHandle, gattAttribute_t *pAttr, uint8_t *pValue, uint16_t *pLen,
                                     uint16_t offset, uint16_t maxLen, uint8_t method);
static bStatus_t ble_uart_WriteAttrCB(uint16_t connHandle, gattAttribute_t *pAttr, uint8_t *pValue, uint16_t len,
...
bStatus_t ble_uart_add_service(ble_uart_ProfileChangeCB_t cb)
{
    ...
    status = GATTServApp_RegisterService(ble_uart_ProfileAttrTbl, GATT_NUM_ATTRS(ble_uart_ProfileAttrTbl),
                                         GATT_MAX_ENCRYPT_KEY_SIZE, &ble_uart_ProfileCBs);
    if (status != SUCCESS)
    ...
    if (pAttr->type.len == ATT_BT_UUID_SIZE)
    {
        ...
        if (uuid == GATT_CLIENT_CHAR_CFG_UUID)
        {
            ...
            if (status == SUCCESS && ble_uart_AppCBs)
            {
                ...
                // PRINT("CCCD set: [%d]\n", charCfg);
                evt.type =
                    (charCfg == GATT_CFG_NO_OPERATION) ? BLE_UART_EVT_TX_NOTI_DISABLED : BLE_UART_EVT_TX_NOTI_ENABLED;
