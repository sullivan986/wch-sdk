...
static std::vector<uint8_t> usb_cdc_send_buff;
static bool usb_cdc_send_ready = false;
void usb_uart_send_data(uint8_t data)
{
    usb_cdc_send_buff.emplace_back(data);
}
...
/* USB工作模式 */
static UINT8 usb_work_mode = USB_CDC_MODE; // USB_VENDOR_MODE;
...
const UINT8 TAB_USB_CDC_DEV_DES[18] = {
    0x12, 0x01, 0x10, 0x01, 0x02, 0x00, 0x00, 0x40, 0x86, 0x1a, 0x40, 0x80, 0x00, 0x30,
    0x01, // 制造者的字符串描述符的索引值
    0x02, // 产品的字符串描述符的索引值
    0x03, // 序号的字符串描述符的索引值
    0x01  // 可能配置的数目
...
const UINT8 TAB_USB_CDC_CFG_DES[] = {
    ...
    0x05, 0x24, 0x00, 0x10, 0x01, 0x04, 0x24, 0x02, 0x02, 0x05, 0x24, 0x06, 0x00, 0x01, 0x05, 0x24, 0x01, 0x01, 0x00,
...
UINT8 TAB_CDC_LINE_CODING[] = {
    0x85,                   /* baud rate*/
    0x20, 0x00, 0x00, 0x00, /* stop bits-1*/
    0x00,                   /* parity - none*/
    0x08                    /* no. of bits 8*/
};
...
const UINT8 TAB_USB_VEN_DEV_DES[] = {0x12, 0x01, 0x10, 0x01, 0xff, 0x00,           0x02, 0x40, // 0x40,
                                     0x86, 0x1a, 0x23, 0x75, 0x00, DEF_IC_PRG_VER,
...
const UINT8 TAB_USB_PRD_STR_DES[] = {0x1e, 0x03, 0x55, 0x00, 0x53, 0x00, 0x42, 0x00, 0x20, 0x00,
                                     0x43, 0x00, 0x44, 0x00, 0x43, 0x00, 0x2d, 0x00, 0x53, 0x00,
...
typedef struct DevInfo
{
    ...
    UINT8
    gUsbFlag; /* USB设备各种操作标志,位0=总线复位,位1=获取设备描述符,位2=设置地址,位3=获取配置描述符,位4=设置配置 */
} DevInfo_Parm;
...
void CH341RegWrite(UINT8 reg_add, UINT8 reg_val)
{
    ...
    case 0x18: // SFR_UART_CTRL -->串口的参数寄存器
    {
        ...
        if (data_bit_val == 0x00)
            data_bit_val = HAL_UART_5_BITS_PER_CHAR;
        else if (data_bit_val == 0x01)
            data_bit_val = HAL_UART_6_BITS_PER_CHAR;
        else if (data_bit_val == 0x02)
            data_bit_val = HAL_UART_7_BITS_PER_CHAR;
        else if (data_bit_val == 0x03)
            data_bit_val = HAL_UART_8_BITS_PER_CHAR;

        stop_bit_val = reg_uart_ctrl & 0x04;
        if (stop_bit_val)
            stop_bit_val = HAL_UART_TWO_STOP_BITS;
        else
            stop_bit_val = HAL_UART_ONE_STOP_BIT;

        parity_val = reg_uart_ctrl & (0x38);
        if (parity_val == 0x00)
            parity_val = HAL_UART_NO_PARITY;
        else if (parity_val == 0x08)
            parity_val = HAL_UART_ODD_PARITY;
        else if (parity_val == 0x18)
            parity_val = HAL_UART_EVEN_PARITY;
        else if (parity_val == 0x28)
            parity_val = HAL_UART_MARK_PARITY;
        else if (parity_val == 0x38)
            parity_val = HAL_UART_SPACE_PARITY;
...
/* 中断处理函数 */
extern "C" __attribute__((interrupt("WCH-Interrupt-fast"))) __attribute__((section(".highcode"))) void USB_IRQHandler(
    void)
{
    UINT8 i;
    UINT8 j;
...
void USB_IRQProcessHandler(void) /* USB中断服务程序 */
{
    ...
    // for(i=0; i<USB_IRQ_FLAG_NUM; i++)
    {
        ...
        if (usb_irq_flag[i])
        {
            ...
            {
                dg_log("UIS_TOKEN_OUT1 usb_rec: ");
                len = usb_irq_len[i];
                // Ep1OUTDataBuf
                for (int i = 0; i < len; i++)
                {
                    usb_cdc_send_buff.emplace_back(Ep1OUTDataBuf[i]);
                ...
                else if (len <= THIS_ENDP0_SIZE) // 上传数据或者状态阶段返回0长度包
                {
                    ...
                    else
                    {
                        if (data_dir == USB_REQ_TYP_IN) // 当前需要上传
                        {
                            ...
                            R8_UEP0_T_LEN = len;
                            R8_UEP0_CTRL =
                                RB_UEP_R_TOG | RB_UEP_T_TOG | UEP_R_RES_NAK | UEP_T_RES_ACK; // 默认数据包是DATA1
                            PFIC_EnableIRQ(USB_IRQn);
                        }
                        else // 当前需要下传
                        {
                            ...
                            R8_UEP0_T_LEN = len;
                            R8_UEP0_CTRL =
                                RB_UEP_R_TOG | RB_UEP_T_TOG | UEP_R_RES_ACK | UEP_T_RES_NAK; // 默认数据包是DATA1
                            PFIC_EnableIRQ(USB_IRQn);
                        }
                    }
                }
                ...
                if (len)
                {
                    if (usb_work_mode == USB_CDC_MODE)
                    {
                        ...
                        case DEF_SET_LINE_CODING: {
                            ...
                            dg_log("LINE_CODING %d %d %d %d %d\r\n", CDCSetSerIdx, (int)set_bps, data_bit, stop_bit,
                                   ver_bit);
    ...
    if ((!usb_cdc_send_buff.empty()) && (usb_cdc_send_ready))
    {
        SendUSBData(usb_cdc_send_buff.data(), usb_cdc_send_buff.size());
        usb_cdc_send_buff.clear();
    }
}
