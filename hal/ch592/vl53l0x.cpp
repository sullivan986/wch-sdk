...
#define checkTimeoutExpired() false //(io_timeout > 0 && ((uint16_t)(millis() - timeout_start_ms) > io_timeout))
      ...
      did_timeout(false)
{
    ...
    if (!init(true))
    {
        log_info("sensor vl53l0x init faild");
    }
}
...
{
    _i2c_device.send_data({{reg, value}});
}

// Write a 16-bit register
void vl53l0x::writeReg16Bit(uint8_t reg, uint16_t value)
{
    _i2c_device.send_data({{reg, (uint8_t)(value >> 8), (uint8_t)value}});
}

// Write a 32-bit register
void vl53l0x::writeReg32Bit(uint8_t reg, uint32_t value)
{
    _i2c_device.send_data(
        {{reg, (uint8_t)(value >> 24), (uint8_t)(value >> 16), (uint8_t)(value >> 8), (uint8_t)value}});
}

// Read an 8-bit register
uint8_t vl53l0x::readReg(uint8_t reg)
{
    return _i2c_device.send_and_recv({{reg}}, 1)[0];
}

// Read a 16-bit register
uint16_t vl53l0x::readReg16Bit(uint8_t reg)
{
    auto read_tmp = _i2c_device.send_and_recv({{reg}}, 2);
    return ((uint16_t)(read_tmp[0] << 8) | (uint16_t)read_tmp[1]);
}
...
{
    auto data = _i2c_device.send_and_recv({{reg}}, count);
