...
#define SUCCESS 0x00
    ...
    typedef struct tag_ble_config
    {
        ...
        pfnTempSampleCB
            tsCB; // Register a program that read the current temperature,determine whether calibration is need
...
#define MS1_TO_SYSTEM_TIME(x)                                                                                          \
    ((x) * 1000 / SYSTEM_TIME_MICROSEN)    // transform unit in ms to unit in 625us ( attentional bias )
...
#define BUILD_UINT32(Byte0, Byte1, Byte2, Byte3)                                                                       \
    ((uint32_t)(((uint32_t)(Byte0) & 0x00FF) + (((uint32_t)(Byte1) & 0x00FF) << 8) +                                   \
...
#define INVALID_MEM_SIZE                                                                                               \
    0x0B //!< The tokens take up too much space and don't fit into Advertisement data and Scan Response Data
...
#define ATT_ERR_UNLIKELY                                                                                               \
    0x0e //!< The attribute request that was requested has encountered an error that was very unlikely, and therefore
...
#define ATT_ERR_UNSUPPORTED_GRP_TYPE                                                                                   \
...
#define ATT_ATTR_HANDLE(info, i) (BUILD_UINT16((info)[ATT_ATTR_HANDLE_IDX((i))], (info)[ATT_ATTR_HANDLE_IDX((i)) + 1]))
#define ATT_GRP_END_HANDLE(info, i)                                                                                    \
    (BUILD_UINT16((info)[ATT_GRP_END_HANDLE_IDX((i))], (info)[ATT_GRP_END_HANDLE_IDX((i)) + 1]))
/** @defgroup ATT_MSG_EVENT_DEFINES ATT Message Event IDs
 * @{
 */
#define ATT_FLOW_CTRL_VIOLATED_EVENT                                                                                   \
    0x7E //!< Sent when ATT flow control is violated on a connection.  This event is sent as an TMOS message defined as
         //!< attFlowCtrlViolatedEvt_t.
#define ATT_MTU_UPDATED_EVENT                                                                                          \
    0x7F //!< Sent when MTU is updated for a connection.  This event is sent as an TMOS message defined as
         //!< attMtuUpdatedEvt_t.
...
#define GAP_DEVICE_INIT_DONE_EVENT                                                                                     \
    0x00 //!< Sent when the Device Initialization is complete.  This event is sent as an tmos message defined as
         //!< gapDeviceInitDoneEvent_t.
#define GAP_DEVICE_DISCOVERY_EVENT                                                                                     \
    0x01 //!< Sent when the Device Discovery Process is complete. This event is sent as an tmos message defined as
         //!< gapDevDiscEvent_t.
#define GAP_ADV_DATA_UPDATE_DONE_EVENT                                                                                 \
    0x02 //!< Sent when the Advertising Data or SCAN_RSP Data has been updated. This event is sent as an tmos message
         //!< defined as gapAdvDataUpdateEvent_t.
#define GAP_MAKE_DISCOVERABLE_DONE_EVENT                                                                               \
    0x03 //!< Sent when the Make Discoverable Request is complete. This event is sent as an tmos message defined as
         //!< gapMakeDiscoverableRspEvent_t.
#define GAP_END_DISCOVERABLE_DONE_EVENT                                                                                \
    0x04 //!< Sent when the Advertising has ended. This event is sent as an tmos message defined as
         //!< gapEndDiscoverableRspEvent_t.
#define GAP_LINK_ESTABLISHED_EVENT                                                                                     \
    0x05 //!< Sent when the Establish Link Request is complete. This event is sent as an tmos message defined as
         //!< gapEstLinkReqEvent_t.
#define GAP_LINK_TERMINATED_EVENT                                                                                      \
    0x06 //!< Sent when a connection was terminated. This event is sent as an tmos message defined as
         //!< gapTerminateLinkEvent_t.
#define GAP_LINK_PARAM_UPDATE_EVENT                                                                                    \
    0x07 //!< Sent when an Update Parameters Event is received. This event is sent as an tmos message defined as
         //!< gapLinkUpdateEvent_t.
#define GAP_RANDOM_ADDR_CHANGED_EVENT                                                                                  \
    0x08 //!< Sent when a random address was changed. This event is sent as an tmos message defined as
         //!< gapRandomAddrEvent_t.
#define GAP_SIGNATURE_UPDATED_EVENT                                                                                    \
    0x09 //!< Sent when the device's signature counter is updated. This event is sent as an tmos message defined as
         //!< gapSignUpdateEvent_t.
#define GAP_AUTHENTICATION_COMPLETE_EVENT                                                                              \
    0x0A //!< Sent when the Authentication (pairing) process is complete. This event is sent as an tmos message defined
         //!< as gapAuthCompleteEvent_t.
#define GAP_PASSKEY_NEEDED_EVENT                                                                                       \
    0x0B //!< Sent when a Passkey is needed.  This is part of the pairing process. This event is sent as an tmos message
         //!< defined as gapPasskeyNeededEvent_t.
#define GAP_SLAVE_REQUESTED_SECURITY_EVENT                                                                             \
    0x0C //!< Sent when a Slave Security Request is received. This event is sent as an tmos message defined as
         //!< gapSlaveSecurityReqEvent_t.
#define GAP_DEVICE_INFO_EVENT                                                                                          \
    0x0D //!< Sent during the Device Discovery Process when a device is discovered. This event is sent as an tmos
         //!< message defined as gapDeviceInfoEvent_t.
#define GAP_BOND_COMPLETE_EVENT                                                                                        \
    0x0E //!< Sent when the bonding process is complete. This event is sent as an tmos message defined as
         //!< gapBondCompleteEvent_t.
#define GAP_PAIRING_REQ_EVENT                                                                                          \
    0x0F //!< Sent when an unexpected Pairing Request is received. This event is sent as an tmos message defined as
         //!< gapPairingReqEvent_t.
#define GAP_DIRECT_DEVICE_INFO_EVENT                                                                                   \
    0x10 //!< Sent when a direct Advertising Data is received. This event is sent as an tmos message defined as
         //!< gapDirectDeviceInfoEvent_t.
#define GAP_PHY_UPDATE_EVENT                                                                                           \
    0x11 //!< Sent when a PHY Update Event is received. This event is sent as an tmos message defined as
         //!< gapLinkUpdateEvent_t.
#define GAP_EXT_ADV_DEVICE_INFO_EVENT                                                                                  \
    0x12 //!< Sent when a Extended Advertising Data is received. This event is sent as an tmos message defined as
         //!< gapExtAdvDeviceInfoEvent_t.
#define GAP_MAKE_PERIODIC_ADV_DONE_EVENT                                                                               \
    0x13 //!< Sent when the Set Periodic Advertising enable is complete. This event is sent as an tmos message defined
         //!< as gapMakePeriodicRspEvent_t.
#define GAP_END_PERIODIC_ADV_DONE_EVENT                                                                                \
    0x14 //!< Sent when the Set Periodic Advertising disable is complete. This event is sent as an tmos message defined
         //!< as gapEndPeriodicRspEvent_t.
#define GAP_SYNC_ESTABLISHED_EVENT                                                                                     \
    0x15 //!< Sent when a Periodic Advertising Sync Establish is complete. This event is sent as an tmos message defined
         //!< as gapSyncEstablishedEvent_t.
#define GAP_PERIODIC_ADV_DEVICE_INFO_EVENT                                                                             \
    0x16 //!< Sent when a Periodic Advertising Data is received. This event is sent as an tmos message defined as
         //!< gapPeriodicAdvDeviceInfoEvent_t.
#define GAP_SYNC_LOST_EVENT                                                                                            \
    0x17 //!< Sent when a Periodic Advertising Sync was lost. This event is sent as an tmos message defined as
         //!< gapSyncLostEvent_t.
#define GAP_SCAN_REQUEST_EVENT                                                                                         \
    0x19 //!< Sent when a SCAN_REQ PDU or an AUX_SCAN_REQ PDU has been received by the advertiser. This event is sent as
         //!< an tmos message defined as gapScanReqReseiveEvent_t.
#define GAP_OOB_NEEDED_EVENT 0x1A //!< resv
#define GAP_MAKE_CONNECTIONESS_CTE_DONE_EVENT                                                                          \
    0x1B //!< Sent when the Set Connectionless CTE Transmit enable is complete. This event is sent as an tmos message
         //!< defined as gapMakeConnectionlessCTERspEvent_t.
#define GAP_END_CONNECTIONESS_CTE_DONE_EVENT                                                                           \
    0x1C //!< Sent when the Set Connectionless CTE Transmit disable is complete. This event is sent as an tmos message
         //!< defined as gapEndConnectionlessCTERspEvent_t.
#define GAP_PERI_ADV_SYNC_TRAN_RECEIVED_EVENT                                                                          \
    0x1D //!< Sent when the periodic advertising sync transfer received. This event is sent as an tmos message defined
         //!< as gapPeriTranReceivec_t.
...
#define GAP_PROFILE_PERIPHERAL                                                                                         \
    0x04 //!< A device that accepts the establishment of an LE physical link using the connection establishment
         //!< procedure
#define GAP_PROFILE_CENTRAL                                                                                            \
    0x08 //!< A device that supports the Central role initiates the establishment of a physical connection
...
#define DUPLICATE_FILTERING_INITIALLY_ENABLED                                                                          \
    (1 << 2) //!< 0: Duplicate filtering initially disabled 1: Duplicate filtering initially enabled
...
// Timers
#define TGAP_GEN_DISC_ADV_MIN                                                                                          \
    0 //!< Minimum time to remain advertising, when in Discoverable mode.Default 0-turns off the timeout. (n * 0.625
      //!< mSec).
#define TGAP_LIM_ADV_TIMEOUT                                                                                           \
    1 //!< Maximum time to remain advertising, when in Limited Discoverable mode.Default 180 seconds. (n * 1 seconds)
#define TGAP_DISC_SCAN                                                                                                 \
    2 //!< Minimum time to perform scanning,Setting this parameter to 0 turns off the timeout.Default 10.24seconds. (n *
      //!< 0.625 mSec)
...
#define TGAP_CONN_EST_HIGH_SCAN_INT                                                                                    \
    11 //!< Scan interval used during Link Layer Initiating state, high duty scan cycle scan parameters (n * 0.625 mSec)
#define TGAP_CONN_EST_HIGH_SCAN_WIND                                                                                   \
    12 //!< Scan window used during Link Layer Initiating state, high duty scan cycle scan parameters (n * 0.625 mSec)
...
#define TGAP_CONN_EST_MIN_CE_LEN                                                                                       \
    15 //!< Local informational parameter about minimum length of connection needed.Default 0. (n * 0.625 mSec)
#define TGAP_CONN_EST_MAX_CE_LEN                                                                                       \
    16 //!< Local informational parameter about maximum length of connection needed.Default 0. (n * 0.625 mSec)

// Proprietary
#define TGAP_PRIVATE_ADDR_INT                                                                                          \
    17 //!< Minimum Time Interval between private (resolvable) address changes.Default 900. (n * 1 seconds)
...
#define TGAP_REJECT_CONN_PARAMS                                                                                        \
    23 //!< Whether or not to reject Connection Parameter Update Request received on Central device. Default FALSE.
#define TGAP_AUTH_TASK_ID 24 //!< Task ID override for Task Authentication control (for stack internal use only)

// v5.x
#define TGAP_ADV_TX_POWER                                                                                              \
    25 //!< Indicates the maximum power level Range: -127 ≤ N ≤ +126 Units: dBm.Default 127(Host has no preference).
#define TGAP_ADV_PRIMARY_PHY                                                                                           \
    26 //!< Indicates the PHY on which the advertising packets are transmitted on the primary advertising channel.LE
       //!< 1M/LE Coded.Default GAP_PHY_VAL_LE_1M.
#define TGAP_ADV_SECONDARY_PHY 27 //!< LE 1M/LE 2M/LE Coded. Default GAP_PHY_VAL_LE_1M.
#define TGAP_ADV_SECONDARY_MAX_SKIP                                                                                    \
    28 //!< Maximum advertising events the Controller can skip before sending the AUX_ADV_IND packets on the secondary
       //!< advertising channel. Default 0.
...
#define TGAP_ADV_ADVERTISING_DURATION                                                                                  \
    31 //!< Advertising duration Range: 0x0001 - 0xFFFF Time = N * 10ms. Default 0-No advertising duration.
#define TGAP_ADV_MAX_EVENTS                                                                                            \
    32 //!< indicates the maximum number of extended advertising events.Range: 0x00 - 0xFF. Default 0(No maximum number
       //!< of advertising events).

// when in General Discovery process
#define TGAP_DISC_SCAN_PHY 33 //!< LE 1M/LE Coded. Default GAP_PHY_BIT_LE_1M.
#define TGAP_DISC_SCAN_CODED_INT                                                                                       \
    34 //!< Scan interval used during Link Layer coded Scanning state, when in General Discovery process (n * 0.625
       //!< mSec)
#define TGAP_DISC_SCAN_CODED_WIND                                                                                      \
    35 //!< Scan window used during Link Layer coded Scanning state, when in General Discovery process (n * 0.625 mSec)
#define TGAP_DISC_SCAN_DURATION                                                                                        \
    36 //!< Scan duration Range: 0x0001 - 0xFFFF Time = N * 10 ms. Default 0-Scan continuously until explicitly disable.
...
#define TGAP_CONN_EST_2M_MIN_CE_LEN                                                                                    \
    43 //!< Local informational parameter about minimum length of connection needed.Default 0. (n * 0.625 mSec)
#define TGAP_CONN_EST_2M_MAX_CE_LEN                                                                                    \
    44 //!< Local informational parameter about maximum length of connection needed.Default 0. (n * 0.625 mSec)
...
#define TGAP_CONN_EST_CODED_SCAN_INT                                                                                   \
    47 //!< Scan interval used during Link Layer Initiating state.Default 16. (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_SCAN_WIND                                                                                  \
    48 //!< Scan window used during Link Layer Initiating state.Default 16. (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_HIGH_SCAN_INT                                                                              \
    49 //!< Scan interval used during Link Layer Initiating state, high duty scan cycle scan parameters (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_HIGH_SCAN_WIND                                                                             \
    50 //!< Scan window used during Link Layer Initiating state, high duty scan cycle scan parameters (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_SUPERV_TIMEOUT 51 //!< Link Layer connection supervision timeout.Default 2000. (n * 10 mSec)
#define TGAP_CONN_EST_CODED_LATENCY                                                                                    \
    52 //!< Link Layer connection slave latency.Default 0. (in number of connection events)
#define TGAP_CONN_EST_CODED_MIN_CE_LEN                                                                                 \
    53 //!< Local informational parameter about minimum length of connection needed.Default 0. (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_MAX_CE_LEN                                                                                 \
    54 //!< Local informational parameter about maximum length of connection needed.Default 0. (n * 0.625 mSec)

// periodic advertising
#define TGAP_PERIODIC_ADV_INT_MIN                                                                                      \
    55 //!< Minimum periodic advertising interval.Range: 0x0006 to 0xFFFF.Default 160. (n * 1.25 mSec)
#define TGAP_PERIODIC_ADV_INT_MAX                                                                                      \
    56 //!< Maximum periodic advertising interval.Range: 0x0006 to 0xFFFF.Default 160. (n * 1.25 mSec)
...
#define TGAP_AFH_CHANNEL_MDOE                                                                                          \
    59 //!< whether t he Controller's channel assessment scheme is enabled or disabled.Default disabled.
...
#define TGAP_LENGTH_OF_SWITCHING_PATTERN                                                                               \
    63 //!< The number of Antenna IDs in the pattern,only used when transmitting an AoD Constant Tone Extension.Default
       //!< 0.
...
#define GAP_ADTYPE_SLAVE_CONN_INTERVAL_RANGE                                                                           \
    0x12 //!< Min and Max values of the connection interval (2 octets Min, 2 octets Max) (0xFFFF indicates no conn
         //!< interval min or max)
...
#define GAPROLE_IRK                                                                                                    \
    0x301 //!< Identity Resolving Key. Read/Write. Size is uint8_t[KEYLEN]. Default is all 0, which means that the IRK
          //!< will be randomly generated.
#define GAPROLE_SRK                                                                                                    \
    0x302 //!< Signature Resolving Key. Read/Write. Size is uint8_t[KEYLEN]. Default is all 0, which means that the SRK
          //!< will be randomly generated.
#define GAPROLE_SIGNCOUNTER 0x303 //!< Sign Counter. Read/Write. Size is uint32_t. Default is 0.
#define GAPROLE_BD_ADDR                                                                                                \
    0x304 //!< Device's Address. Read Only. Size is uint8_t[B_ADDR_LEN]. This item is read from the controller.
#define GAPROLE_ADVERT_ENABLED                                                                                         \
    0x305                         //!< Enable/Disable Advertising. Read/Write. Size is uint8_t. Default is TRUE=Enabled.
#define GAPROLE_ADVERT_DATA 0x306 //!< Advertisement Data. Read/Write. Max size is B_MAX_ADV_EXT_LEN. Default to all 0.
#define GAPROLE_SCAN_RSP_DATA                                                                                          \
    0x307 //!< Scan Response Data. Read/Write. Max size is B_MAX_ADV_EXT_LEN. Defaults to all 0.
#define GAPROLE_ADV_EVENT_TYPE                                                                                         \
    0x308 //!< Advertisement Type. Read/Write. Size is uint8_t.  Default is GAP_ADTYPE_ADV_IND.
#define GAPROLE_ADV_DIRECT_TYPE                                                                                        \
    0x309 //!< Direct Advertisement Address Type. Read/Write. Size is uint8_t. Default is ADDRTYPE_PUBLIC.
#define GAPROLE_ADV_DIRECT_ADDR                                                                                        \
    0x30A //!< Direct Advertisement Address. Read/Write. Size is uint8_t[B_ADDR_LEN]. Default is NULL.
#define GAPROLE_ADV_CHANNEL_MAP                                                                                        \
    0x30B //!< Which channels to advertise on. Read/Write Size is uint8_t. Default is GAP_ADVCHAN_ALL
#define GAPROLE_ADV_FILTER_POLICY                                                                                      \
    0x30C //!< Filter Policy. Ignored when directed advertising is used. Read/Write. Size is uint8_t. Default is
          //!< GAP_FILTER_POLICY_ALL.
#define GAPROLE_STATE                                                                                                  \
    0x30D //!< Reading this parameter will return GAP Peripheral Role State. Read Only. Size is uint8_t.
#define GAPROLE_MAX_SCAN_RES 0x30E //!< Maximum number of discover scan results to receive. Default is 0 = unlimited.
#define GAPROLE_MIN_CONN_INTERVAL                                                                                      \
    0x311 //!< Minimum Connection Interval to allow (n * 1.25ms).  Range: 7.5 msec to 4 seconds (0x0006 to 0x0C80).
          //!< Read/Write. Size is uint16_t. Default is 7.5 milliseconds (0x0006).
#define GAPROLE_MAX_CONN_INTERVAL                                                                                      \
    0x312 //!< Maximum Connection Interval to allow (n * 1.25ms).  Range: 7.5 msec to 4 seconds (0x0006 to 0x0C80).
          //!< Read/Write. Size is uint16_t. Default is 4 seconds (0x0C80).
// v5.x
#define GAPROLE_PHY_TX_SUPPORTED                                                                                       \
    0x313 //!< The transmitter PHYs that the Host prefers the Controller to use.Default is GAP_PHY_BIT_ALL
#define GAPROLE_PHY_RX_SUPPORTED                                                                                       \
...
#define GAPROLE_CTE_CONNECTIONLESS_ENABLED                                                                             \
    0x317 //!< Enable/Disable Connectionless CTE Transmit. Read/Write. Size is uint8_t. Default is FALSE=Disable.

/************************************GAPBOND***********************************/
// GAPBOND_PROFILE_PARAMETERS GAP Bond Manager Parameters
#define GAPBOND_PERI_PAIRING_MODE                                                                                      \
    0x400 //!< Pairing Mode: @ref  GAPBOND_PAIRING_MODE_DEFINES. Read/Write. Size is uint8_t. Default is
          //!< GAPBOND_PAIRING_MODE_WAIT_FOR_REQ.
#define GAPBOND_PERI_MITM_PROTECTION                                                                                   \
    0x401 //!< Man-In-The-Middle (MITM) basically turns on Passkey protection in the pairing algorithm. Read/Write. Size
          //!< is uint8_t. Default is 0(disabled).
#define GAPBOND_PERI_IO_CAPABILITIES                                                                                   \
    0x402 //!< I/O capabilities.  Read/Write. Size is uint8_t. Default is GAPBOND_IO_CAP_DISPLAY_ONLY @ref
          //!< GAPBOND_IO_CAP_DEFINES.
#define GAPBOND_PERI_OOB_ENABLED                                                                                       \
    0x403 //!< OOB data available for pairing algorithm. Read/Write. Size is uint8_t. Default is 0(disabled).
#define GAPBOND_PERI_OOB_DATA 0x404 //!< OOB Data. Read/Write. size uint8_t[16]. Default is all 0's.
#define GAPBOND_PERI_BONDING_ENABLED                                                                                   \
    0x405 //!< Request Bonding during the pairing process if enabled.  Read/Write. Size is uint8_t. Default is
          //!< 0(disabled).
#define GAPBOND_PERI_KEY_DIST_LIST                                                                                     \
    0x406 //!< The key distribution list for bonding.  size is uint8_t.  @ref GAPBOND_KEY_DIST_DEFINES. Default is 0x77.
#define GAPBOND_PERI_DEFAULT_PASSCODE                                                                                  \
    0x407 //!< The default passcode for MITM protection. size is uint32_t. Range is 0 - 999,999. Default is 0.
#define GAPBOND_CENT_PAIRING_MODE                                                                                      \
    0x408 //!< Pairing Mode: @ref  GAPBOND_PAIRING_MODE_DEFINES. Read/Write. Size is uint8_t. Default is
          //!< GAPBOND_PAIRING_MODE_WAIT_FOR_REQ.
#define GAPBOND_CENT_MITM_PROTECTION                                                                                   \
    0x409 //!< Man-In-The-Middle (MITM) basically turns on Passkey protection in the pairing algorithm. Read/Write. Size
          //!< is uint8_t. Default is 0(disabled).
#define GAPBOND_CENT_IO_CAPABILITIES                                                                                   \
    0x40A //!< I/O capabilities.  Read/Write. Size is uint8_t. Default is GAPBOND_IO_CAP_DISPLAY_ONLY @ref
          //!< GAPBOND_IO_CAP_DEFINES.
#define GAPBOND_CENT_OOB_ENABLED                                                                                       \
    0x40B //!< OOB data available for pairing algorithm. Read/Write. Size is uint8_t. Default is 0(disabled).
#define GAPBOND_CENT_OOB_DATA 0x40C //!< OOB Data. Read/Write. size uint8_t[16]. Default is all 0's.
#define GAPBOND_CENT_BONDING_ENABLED                                                                                   \
    0x40D //!< Request Bonding during the pairing process if enabled.  Read/Write. Size is uint8_t. Default is
          //!< 0(disabled).
#define GAPBOND_CENT_KEY_DIST_LIST                                                                                     \
    0x40E //!< The key distribution list for bonding.  size is uint8_t.  @ref GAPBOND_KEY_DIST_DEFINES. Default is 0x77.
#define GAPBOND_CENT_DEFAULT_PASSCODE                                                                                  \
    0x40F //!< The default passcode for MITM protection. size is uint32_t. Range is 0 - 999,999. Default is 0.
#define GAPBOND_ERASE_ALLBONDS 0x410 //!< Erase all of the bonded devices. Write Only. No Size.
#define GAPBOND_AUTO_FAIL_PAIRING                                                                                      \
    0x411 //!< TEST MODE (DO NOT USE) to automatically send a Pairing Fail when a Pairing Request is received.
          //!< Read/Write. size is uint8_t. Default is 0 (disabled).
#define GAPBOND_AUTO_FAIL_REASON                                                                                       \
    0x412 //!< TEST MODE (DO NOT USE) Pairing Fail reason when auto failing. Read/Write. size is uint8_t. Default is
...
#define GAPBOND_AUTO_SYNC_WL                                                                                           \
    0x414 //!< Clears the White List adds to it each unique address stored by bonds in NV. Read/Write. Size is uint8_t.
          //!< Default is FALSE.
#define GAPBOND_BOND_COUNT                                                                                             \
    0x415 //!< Gets the total number of bonds stored in NV. Read Only. Size is uint8_t. Default is 0 (no bonds).
#define GAPBOND_BOND_FAIL_ACTION                                                                                       \
    0x416 //!< Possible actions Central may take upon an unsuccessful bonding. Write Only. Size is uint8_t. Default is
          //!< 0x02 (Terminate link upon unsuccessful bonding).
#define GAPBOND_ERASE_SINGLEBOND                                                                                       \
    0x417 //!< Erase a single bonded device. Write only. Must provide address type followed by device address.
...
#define GAPBOND_DISABLE_SINGLEBOND                                                                                     \
    0x41A //!< Disable a single bonded device. Write only. Must provide address type followed by device address.
#define GAPBOND_ENABLE_SINGLEBOND                                                                                      \
    0x41B //!< Ensable a single bonded device. Write only. Must provide address type followed by device address.
...
#define GAPBOND_ERASE_AUTO                                                                                             \
    0x41E //!< Auto erase all of the bonded devices when the maximum number is reached.Size is uint8_t. Default is
          //!< 1(enabled).
#define GAPBOND_AUTO_SYNC_RL                                                                                           \
    0x41F //!< Clears the Resolving List adds to it each unique address stored by bonds in NV. Read/Write. Size is
          //!< uint8_t. Default is FALSE.
...
#define SMP_PAIRING_FAILED_PASSKEY_ENTRY_FAILED                                                                        \
...
#define SMP_PAIRING_FAILED_AUTH_REQ                                                                                    \
    0x03 //!< The pairing procedure can't be performed as authentication requirements can't be met due to IO
...
#define SMP_PAIRING_FAILED_ENC_KEY_SIZE                                                                                \
...
#define SMP_PAIRING_FAILED_REPEATED_ATTEMPTS                                                                           \
    0x09 //!< Pairing or authentication procedure is disallowed because too little time has elapsed since the last
         //!< pairing request or security request.
#define SMP_PAIRING_FAILED_INVALID_PARAMERERS                                                                          \
    0x0A //!< The Invalid Parameters error code indicates that the command length is invalid or that a parameter is
         //!< outside of the specified range.
#define SMP_PAIRING_FAILED_DHKEY_CHECK_FAILED                                                                          \
    0x0B //!< Indicates to the remote device that the DHKey Check value received doesn’t match the one calculated by the
         //!< local device.
#define SMP_PAIRING_FAILED_NUMERIC_COMPARISON                                                                          \
    0x0C //!< Indicates that the confirm values in the numeric comparison protocol do not match.
...
#define GAPBOND_FAIL_TERMINATE_ERASE_BONDS                                                                             \
    0x03 //!< Terminate link and erase all existing bonds on device upon unsuccessful bonding
...
#define BLE_NVID_MAX_VAL 0x7F
    ...
    typedef struct
    {
        uint8_t eventType;        //!< Indicates advertising event type used by the advertiser: @ref
    ...
    typedef int (*pfnEcc_dhkey_t)(uint8_t *peer_pub_key_x, uint8_t *peer_pub_key_y, uint8_t *our_priv_key,
                                  uint8_t *out_dhkey);

    typedef int (*pfnEcc_alg_f4_t)(uint8_t *u, uint8_t *v, uint8_t *x, uint8_t z, uint8_t *out_enc_data);

    typedef int (*pfnEcc_alg_g2_t)(uint8_t *u, uint8_t *v, uint8_t *x, uint8_t *y, uint32_t *passkey);

    typedef int (*pfnEcc_alg_f5_t)(uint8_t *w, uint8_t *n1, uint8_t *n2, uint8_t a1t, uint8_t *a1, uint8_t a2t,
                                   uint8_t *a2, uint8_t *mackey, uint8_t *ltk);

    typedef int (*pfnEcc_alg_f6_t)(uint8_t *w, uint8_t *n1, uint8_t *n2, uint8_t *r, uint8_t *iocap, uint8_t a1t,
                                   uint8_t *a1, uint8_t a2t, uint8_t *a2, uint8_t *check);
...
#define GAPROLE_CTE_T_STATE_VALID (1 << 25)    //!< indicates Connectionless CTE Transmit states change
    ...
    typedef union {
        ...
        gapExtAdvDeviceInfoEvent_t
            deviceExtAdvInfo;                 //!< Discovery extend advertising device information event structure.
    ...
    typedef struct
    {
        uint8_t eventType; //!< Indicates advertising event type used by the advertiser: @ref
                           //!< GAP_ADVERTISEMENT_REPORT_TYPE_DEFINES
        uint8_t addrType;  //!< Scan Address Type:0x00-Public Device Address or Public Identity Address 0x01-Random
    ...
    typedef struct
    {
        ...
        /*
         bit0: used to determine whether the Periodic  Advertiser List is used
           0: Use the Advertising_SID, Advertisier_Address_Type, and Advertiser_Address parameters to determine which
         advertiser to listen to. 1: Use the Periodic Advertiser List to determine which advertiser to listen to. bit1:
         whether GAP_PERIODIC_ADV_DEVICE_INFO_EVENT events for this periodic advertising train are initially enabled or
         disabled. 0: Reporting initially enabled 1: Reporting initially disabled bit2: 0: Duplicate filtering initially
         disabled 1: Duplicate filtering initially enabled */
        ...
        uint8_t addr[B_ADDR_LEN]; //!< Device's Address
        uint16_t skip; //!< the maximum number of consecutive periodic advertising events that the receiver may skip
                       //!< after successfully receiving a periodic advertising packet.Range: 0x0000 to 0x01F3
        uint16_t syncTimeout; //!< the maximum permitted time between successful receives. If this time is exceeded,
                              //!< synchronization is lost. Time = N*10 ms.Range: 0x000A to 0x4000
        uint8_t syncCTEType; //!< specifies whether to only synchronize to periodic advertising with certain types of
                             //!< Constant Tone Extension (a value of 0 indicates that the presence or absence of a
...
#define LLE_MODE_AUTO                                                                                                  \
    (1) //!< auto mode, auto swtich to the receiving status after sending and the sending status after receiving
...
#define linkDB_Up(connectionHandle) linkDB_State((connectionHandle), LINK_CONNECTED)
     ...
     */
    extern bStatus_t GATT_Indication(uint16_t connHandle, attHandleValueInd_t *pInd, uint8_t authenticated,
                                     uint8_t taskId);
    /**
     * @brief   This sub-procedure is used when a server is configured to
     ...
     *          bleTimeout: Previous transaction timed out.<BR>
     */
    extern bStatus_t GATT_DiscAllCharDescs(uint16_t connHandle, uint16_t startHandle, uint16_t endHandle,
                                           uint8_t taskId);
     ...
     *          bleIncorrectMode: No connection to update.<BR>
     */
    extern bStatus_t GAPRole_UpdatePHY(uint16_t connHandle, uint8_t all_phys, uint8_t tx_phys, uint8_t rx_phys,
